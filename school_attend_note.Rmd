...
prepost.sth.long.data %>% 
  ...
  (l(ready.data ~ {
    ...
    ggplot(ready.data, aes(x=survey, group=dep.var)) + 
      geom_bar(aes(y=fit, fill=dep.var), stat="identity", position=dodge, alpha=0.5, width=0.5) +
      geom_errorbar(aes(ymin=fit.min, ymax=fit.max), position=dodge, width=0.3) +
      scale_y_continuous("Proportion") + 
      scale_x_discrete("Survey") + 
      scale_fill_discrete("Proportion of Infection", labels=c("Moderate and High Intensity", "Incidence")) +
...
prepost.sth.long.data %>% 
  (l(.data ~ {
    filter(.data, infection.type == "as", county %in% western.province) %>%
    rq(epg ~ survey, tau=tau.range, data=.) %>%
    ...
    annotate("text", x=4, y=5000, label="Moderate Intensity", size=4, vjust=-0.8, hjust=0.2) +
