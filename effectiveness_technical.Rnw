...
\title{Technical Notes on Evaluating the Effectiveness of MDA Mechanisms}
...
Since we make the assumption that all those receiving treatment are healthy, $H_{ij}(1) = 1$, we only need to collect parasitology surveys from those refusing treatment, $D_{ij} = 0$.
